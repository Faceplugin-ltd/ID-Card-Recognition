...
# Signature of the last license file we passed to activate(). Used so replacing
# license.txt while the API is running picks up the new key without a restart.
...
def remember_loaded(root: Path) -> None:
    global _loaded_sig
    path = find_license(root)
    _loaded_sig = _file_sig(path) if path is not None else None


def reload_if_changed(root: Path, activate: Callable[[str], int]) -> None:
    """Re-run activate() when LICENSE / license.txt content changes on disk."""
    global _loaded_sig
    path = find_license(root)
    if path is None:
        return
